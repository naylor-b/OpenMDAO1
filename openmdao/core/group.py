...
class Group(System):
    ...
    def assemble_sparse_jacobian(self, mode='fwd', method='assemble', mult=None,
                                 solve_method='solve'):
        ...
        if method == 'MVP':
            ...
            # partials is still currently dense...
            partials = np.empty((n_edge, n_edge))
            ...
            for i in range(n_edge):
                ...
                # assignment operation could end up being inefficient if partials
                # is a sparse matrix, and we may be ahead just to create a new
                # partials maxtrix each time using bmat or something.
                partials[:, i] = mult(ident_vec)
        ...
        # Assemble the Jacobian
            ...
            icache = self._icache
            conn = self.connections
            prom_names = self._sysdata.to_prom_name
